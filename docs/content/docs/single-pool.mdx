...
Internally, all versions of the `deposit` command/transaction assign stake account authority to Single Pool in a prior instruction.
This means the user does not have to provide a wallet signature to the Single Pool program and is intended as a safety measure under the principle of least privilege.
...
Stake is split into to a new stake account pre-funded with rent-exemption, with all authority assigned to the user wallet.

Internally, all versions of the `withdraw` command/transaction grant a token delegation to the Single Pool authority in a prior instruction to accomplish the burn.
This means the user does not have to provide a wallet signature to the Single Pool program and is intended as a safety measure under the principle of least privilege.
