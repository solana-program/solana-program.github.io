...
description: Create, read, advance, and close SPL Nonce accounts.
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs';
import { NonceAdvanceDiagram } from '@/components/programmatic-signer/nonce-advance';

An SPL Nonce account stores a hash that advances each time an authorized action succeeds. It also records the authority allowed to advance it and the slot it was initialized in. An authority can hold as many nonce accounts as it needs. Each one advances independently of the others.
...
Fetch the account, check that SPL Nonce owns it and that it holds exactly 72 bytes, then read the three fields.
